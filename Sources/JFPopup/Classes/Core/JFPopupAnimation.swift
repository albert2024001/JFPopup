...
class JFPopupAnimation: NSObject {
    static func present(with transitonContext: UIViewControllerContextTransitioning?, config: JFPopupConfig, contianerView: UIView, completion: ((_ isFinished: Bool) -> Void)?) {
    ...
    static func dismiss(with transitonContext: UIViewControllerContextTransitioning?, config: JFPopupConfig, contianerView: UIView?, completion: ((_ isFinished: Bool) -> Void)?) {
        let type = config.animationType
        switch type {
        case .bottomSheet:
            do {
