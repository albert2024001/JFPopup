...
public typealias TapActionCallBack = () -> Void
...
class JFAlertAction: NSObject {
    ...
    var defaultColor: UIColor = .init(red: 20 / 255.0, green: 20 / 255.0, blue: 20 / 255.0, alpha: 1)
    ...
    func buildActionButton() -> UIButton? {
        ...
        for option in options {
            switch option {
            case let .textColor(uIColor):
                color = uIColor
            case let .text(t):
                text = t
            case let .tapActionCallback(tap):
                tapActionCallback = tap
            }
        }
